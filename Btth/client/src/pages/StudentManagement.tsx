...
import { getAllUsers, updateUserStatus } from "../apis/UserApi";
import LoadingProcess from "../components/LoadingProcess.tsx";
import { Link } from "react-router-dom";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import {faUsers, faGraduationCap, faCog, faSignOutAlt, faBars, faBell, faChevronRight, faPlusSquare, faSearch, faFilter,} from "@fortawesome/free-solid-svg-icons";
...
import {restoreFromStorage} from "../slices/UserSlice.ts";
// import Avatar from "../assets/avatar.svg";
// import {setUsers} from "../slices/UserSlice.ts";
// import { setUsers } from "../apis/UserApi"; // Import setUsers từ userSlice

// Memoized selector để lấy state.user
...
const StudentManagement = () => {
    ...
    // const { users: allUsers } = useAppSelector(selectUserState);
    ...
    const filteredUsers = users
        .filter((user: User) => {
            ...
            if (!isUser) console.log("Filtered out admin:", user);
            return isUser;
        })
        .map((user: User) => ({
            ...user,
        ...
        if (selectedUser) {
            await dispatch(
                ...
                if (updateUserStatus.fulfilled.match(action)) {
                    ...
                    console.log("Status updated, new users:", action.payload);
                } else {
                    console.error("Failed to update user status:", action.payload);
                }
            });
        }
    };
    ...
    return (
        <div
            className={`font-poppins m-0 p-0 box-border transition-colors duration-300 ${
                darkMode ? "bg-[#37353E] text-[#FBFBFB]" : "bg-[#EEEEEE] min-h-screen text-[#37353E]"
            }`}
        >
            ...
            >
                ...
                <div className="menu top">
                    ...
                    <div className="menu-item">
                        ...
                        <Link
                            to="/testManagement"
                            className={`menu-link group ${sidebarOpen ? "" : "hide"} ${darkMode ? "dark" : ""}`}
                        >
                            <svg
                                className="min-w-[40px] flex justify-center group-hover:fill-[#6B7597] transition-colors duration-200"
                                xmlns="http://www.w3.org/2000/svg"
            ...
            >
                {/* NAVBAR */}
                <nav
                    className={`h-14 px-6 flex items-center gap-6 font-lato sticky top-0 left-0 z-[1000] ${
                ...
                >
                    ...
                    <div className="flex items-center gap-[10px] ml-auto mr-5">
                        ...
                        <Link
                            to="#"
                            className={`text-xl relative ${darkMode ? "text-[#FBFBFB]" : "text-[#342E37]"}`}
                        >
                    ...
                    <div className="flex items-center justify-between gap-4 flex-wrap">
                        <div>
                            ...
                            <div className="flex items-center gap-2">
                                <span className="text-gray-400 pointer-events-none">Student</span>
                        ...
                        <div className="flex gap-2">
                            <button
                                ...
                                className="px-4 py-2 rounded-md cursor-pointer text-base bg-[#6B7597] text-white transition-all duration-300 hover:bg-[#565D7C] hover:text-white flex items-center gap-2"
                            >
                    ...
                    <form className="w-full mr-auto mt-5">
                        <div className="flex items-center h-9">
                            <input
                                type="search"
                                placeholder="Search..."
                                className={`max-[576px]:hidden flex-grow px-4 h-full border-2 rounded-l-[36px] outline-none w-full transition-colors duration-300
                                    ${darkMode
                            ...
                            <button
                                type="submit"
                                className={`w-9 h-full flex justify-center items-center text-lg border-none outline-none rounded-r-[36px] cursor-pointer transition-colors duration-300 max-[576px]:bg-transparent max-[576px]:text-[#342E37] max-[576px]:w-auto max-[576px]:h-auto form-[.show]:w-9 form-[.show]:h-full form-[.show]:bg-[#DB504A] form-[.show]:text-white form-[.show]:rounded-r-[36px]
                                    ${darkMode
                            ...
                            <section className="content">
                                <div className="lg:px-12 px-3">
                                    ...
                                    {!loading && filteredUsers.length === 0 && !error && (
                                        <div className={darkMode ? "text-gray-400" : "text-gray-500"}>No users available</div>
                                    )}
