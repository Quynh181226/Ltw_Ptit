...
const TableStudent = ({ users, setCode, setSelectedUser, setOpenAddEdit, setOpenDelete, setOpenStatus, darkMode }: TableStudentProps) => {
    ...
    const getStatusText = (user: User) => {
        if (user.lockUntil && Number(user.lockUntil) > Date.now()) return "Locked";
    ...
    const getStatusClass = (user: User) => {
        if (user.lockUntil && Number(user.lockUntil) > Date.now()) {
            return `w-[80%] text-center px-2 py-[2px] rounded-2xl border-2 cursor-pointer font-bold bg-[#eceff1] border-[#90a4ae] text-[#607d8b] hover:bg-[#607d8b] hover:text-white transition duration-200`;
        }
        if (user.suspended) {
            return `w-[80%] text-center px-2 py-[2px] rounded-2xl border-2 cursor-pointer font-bold bg-[#fbeaea] border-[#e57373] text-[#d04444] hover:bg-[#d04444] hover:text-white transition duration-200`;
        }
        if (user.active) {
            return `w-[80%] text-center px-2 py-[2px] rounded-2xl border-2 cursor-pointer bg-[#e9f6ec] border-[#90a56a] text-[#90a56a] hover:bg-[#6f8a47] hover:text-white transition duration-200`;
        }
        if (user.waiting) {
            return `w-[80%] text-center px-2 py-[2px] rounded-2xl border-2 cursor-pointer uppercase bg-[#fcf6e8] border-[#e6b84f] text-[#eab541] hover:bg-[#d5c296] hover:text-white transition duration-200`;
        }
        return `w-[80%] text-center px-2 py-[2px] rounded-2xl border-2 cursor-pointer bg-[#f1f5f9] border-[#d1d5db] text-[#6b7280] hover:bg-[#e2e8f0] transition duration-200`;
    };
