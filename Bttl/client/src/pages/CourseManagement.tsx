...
import {faBars, faBell, faChevronRight, faPlusSquare, faSearch, faFilter, faUsers, faGraduationCap, faCog, faSignOutAlt,} from '@fortawesome/free-solid-svg-icons';
import { Dropdown, Menu } from 'antd';
import { useAppDispatch, useAppSelector } from '../hooks/Hook';
import { getAllCourses, addCourse } from "../apis/CourseApi.ts";
...
const CourseManagement = () => {
    ...
    const { list, status, error } = useAppSelector((state) => state.courses);
    useEffect(() => {
            ...
            >
                ...
                <main className="w-full min-h-screen p-[36px_24px] font-[Poppins,sans-serif] max-h-[calc(100vh-56px)] overflow-y-auto">
                    <div className="flex items-center justify-between gap-4 flex-wrap">
                        <div>
                            <h1 className="text-[36px] font-semibold mb-[10px]">Course Management</h1>
                            <div className="flex items-center gap-4">
                        ...
                        >
                            ...
                            <section className="content">
                                <div className="lg:px-30 px-12">
                                    ...
                                    {error && <div>{error}</div>}
                                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 md:gap-8 xl:gap-10 w-full max-w-[1300px]  mx-auto">
                                        {list.map((course: any) => (
                                            <CourseCard
                                                key={course.id}
                                                course={course}
                                    ...
                                    <ModalAddCourse
                                        ...
                                        onSubmit={(title: string) => {
                                            dispatch(addCourse({ title, lessons: [], numberLessons: 0, status: false }));
                                            setOpenAdd(false);
                                        }}
                                    />
                                </div>
